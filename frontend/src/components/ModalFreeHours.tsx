...
export default function ModalFreeHours({ active, setActive, tag, idCourse }: IProps) {
    ...
    getCourses()
      .then((res) => {
        ...
        for (let i = 0; i < newOptions.length; i++) {
          if (newOptions[i].value === idCourse) {
            setSelectedOption(newOptions[i])
            break
          }
        }
      })
  ...
  return (
    ...
    >
      ...
      <motion.div initial={{ opacity: 0 }} transition={{ duration: 0.5 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative flex flex-col h-full w-full md:flex-row">
        ...
        <img src={folder} alt="" className="hidden md:block absolute scale-[47%] top-[-110px] right-[-300px]" />
        ...
        <div className="flex flex-col justify-between h-full max-w-[780px] px-[20px] md:px-0 w-full mx-auto pt-[1px] z-10">
          <div className="pb-[20px]">
            <article className="mb-[30px] max-w-[1220px] mx-auto">
              <span className="block font-semibold italic text-[16px] mt-[40px] md:mt-[10px] mb-[12px] l:mb-[20px] text-[#00BA61]">Акция!</span>
              <h2 className="font-black text-white text-[28px] mb-[15px] l:mb-[30px] l:text-[48px]">Получите 2 бесплатных часа, <br /> и узнайте, подходит ли <br /> формат курса вам!</h2>
            ...
            <form onSubmit={handleSubmit((data) => { sendRequest(data) })}>
              ...
              <button type={'submit'} disabled={!isValid} className="hidden button-default bg-[#00BA61] hover:bg-[#26c97b] disabled:hover:bg-[#00BA61] px-[30px] py-[20px] mt-[18px]  min-h-[58px] rounded-[8px] font-semibold text-white text-[18px] text-center w-full md:block">Записаться бесплатно</button>
          ...
          <button onClick={handleSubmit((data) => { sendRequest(data) })} type={'submit'} disabled={!isValid} className="block button-default bg-[#00BA61] hover:bg-[#26c97b] disabled:hover:bg-[#00BA61] px-[30px] py-[20px] mt-[18px] min-h-[58px] rounded-[8px] font-semibold text-white text-[18px] text-center w-full md:hidden">Записаться бесплатно</button>
