...
export default function ModalConsulting({ active, setActive, tag, idCourse }: IProps) {
  ...
  async function sendRequest(data: any) {
    ...
    if (idCourse) { postData.course = idCourse }
        ...
        <div className="flex flex-col justify-between h-full max-w-[780px] px-[20px] md:px-0 w-full mx-auto pt-[1px] z-10 ">
          <div className="pb-[20px]">
            <article className="mb-[30px] max-w-[1220px] mx-auto">
              <span className="block font-semibold italic text-[16px] mt-[40px] md:mt-[10px] mb-[12px] l:mb-[20px] text-[#00BA61]">Бесплатная консультация</span>
              <h2 className="font-black text-white text-[28px] mb-[15px] l:mb-[30px] l:text-[48px]">Хотите узнать <br /> о курсе подробнее?</h2>
            ...
            <form onSubmit={handleSubmit((data) => { sendRequest(data) })}>
              ...
              <Select components={{
                ...
                defaultValue={{ value: 0, label: 'Выбрать курс' }}
          ...
          <button onClick={handleSubmit((data) => { sendRequest(data) })} type={'submit'} disabled={!isValid} className="block button-default bg-[#00BA61] hover:bg-[#26c97b]  px-[30px] py-[20px] mt-[18px] min-h-[58px] rounded-[8px] font-semibold text-white text-[18px] text-center w-full md:hidden">Записаться бесплатно</button>
